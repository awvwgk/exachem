...
public:
  // Constructor
  CCSD_T_Fused_Common()  = default;
  ~CCSD_T_Fused_Common() = default;

  // Copy constructor
  CCSD_T_Fused_Common(const CCSD_T_Fused_Common&) = default;
  // Copy assignment operator
  CCSD_T_Fused_Common& operator=(const CCSD_T_Fused_Common&) = default;

  // Move constructor
  CCSD_T_Fused_Common(CCSD_T_Fused_Common&&) = default;
  // Move assignment operator
  CCSD_T_Fused_Common& operator=(CCSD_T_Fused_Common&&) = default;
  ...
  long double ccsd_t_fully_fused_performance(
    ...
    for(unsigned int current_id = init_id; current_id < list_tasks.size();
      ...
      ccsd_t_data_s1_info_only(is_restricted, noab, nvab, k_spin, k_evl_sorted, k_range, t_h1b,
                               t_h2b, t_h3b, t_p4b, t_p5b, t_p6b, df_simple_s1_size,
                               df_simple_s1_exec, &num_s1_enabled_kernels, total_comm_data);

      ccsd_t_data_d1_info_only(is_restricted, noab, nvab, k_spin, k_evl_sorted, k_range, t_h1b,
                               t_h2b, t_h3b, t_p4b, t_p5b, t_p6b, df_simple_d1_size,
                               df_simple_d1_exec, &num_d1_enabled_kernels, total_comm_data);

      ccsd_t_data_d2_info_only(is_restricted, noab, nvab, k_spin, k_evl_sorted, k_range, t_h1b,
                               t_h2b, t_h3b, t_p4b, t_p5b, t_p6b, df_simple_d2_size,
                               df_simple_d2_exec, &num_d2_enabled_kernels, total_comm_data);
...
inline void helper_calculate_num_ops(const Index noab, const Index nvab, int* df_simple_s1_size,
                                     ...
                                     long double& total_num_ops_s1, long double& total_num_ops_d1,
                                     long double& total_num_ops_d2) {
  CCSD_T_Fused_Common<double> ccsd_t_obj;
  ccsd_t_obj.helper_calculate_num_ops(
    noab, nvab, df_simple_s1_size, df_simple_d1_size, df_simple_d2_size, df_simple_s1_exec,
    df_simple_d1_exec, df_simple_d2_exec, task_num_ops_s1, task_num_ops_d1, task_num_op_sd2,
...
long double ccsd_t_fully_fused_performance(
  ...
  int64_t rank, int task_stride, const Index noab, const Index nvab, std::vector<int>& k_spin,
  std::vector<size_t>& k_range, std::vector<size_t>& k_offset, std::vector<T>& k_evl_sorted) {
  CCSD_T_Fused_Common<T> ccsd_t_obj;
  return ccsd_t_obj.ccsd_t_fully_fused_performance(is_restricted, list_tasks, rank, task_stride,
                                                   noab, nvab, k_spin, k_range, k_offset,
                                                   k_evl_sorted);
}
