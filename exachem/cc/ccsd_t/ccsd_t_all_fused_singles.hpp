...
template<typename T>
class CCSD_T_Fused_Singles {
public:
  CCSD_T_Fused_Singles()          = default;
  virtual ~CCSD_T_Fused_Singles() = default;

  // Copy constructor and copy assignment operator
  CCSD_T_Fused_Singles(const CCSD_T_Fused_Singles&)            = default;
  CCSD_T_Fused_Singles& operator=(const CCSD_T_Fused_Singles&) = default;

  // Move constructor and move assignment operator
  CCSD_T_Fused_Singles(CCSD_T_Fused_Singles&&)            = default;
  CCSD_T_Fused_Singles& operator=(CCSD_T_Fused_Singles&&) = default;
...
void ccsd_t_data_s1_new(bool is_restricted,
                        ...
                        LRUCache<Index, std::vector<T>>& cache_s1t,
                        LRUCache<Index, std::vector<T>>& cache_s1v) {
  CCSD_T_Fused_Singles<T> ccsd_t_obj;
  ccsd_t_obj.ccsd_t_data_s1_new(is_restricted, noab, nvab, k_spin, d_t1, d_t2, d_v2, k_evl_sorted,
                                k_range, t_h1b, t_h2b, t_h3b, t_p4b, t_p5b, t_p6b, size_T_s1_t1,
                                size_T_s1_v2, df_simple_s1_size, df_simple_s1_exec, df_T_s1_t1,
                                df_T_s1_v2, df_num_s1_enabled, cache_s1t, cache_s1v);
}
...
void ccsd_t_data_s1_info_only(bool is_restricted, const Index noab, const Index nvab,
                              ...
                              int* num_enabled_kernels, size_t& comm_data_elems) {
  CCSD_T_Fused_Singles<T> ccsd_t_obj;
  ccsd_t_obj.ccsd_t_data_s1_info_only(is_restricted, noab, nvab, k_spin, k_evl_sorted, k_range,
                                      t_h1b, t_h2b, t_h3b, t_p4b, t_p5b, t_p6b, df_simple_s1_size,
                                      df_simple_s1_exec, num_enabled_kernels, comm_data_elems);
}
