...
std::tuple<T, T> exachem::scf::SCFIter<T>::scf_iter_body(
  ...
  Tensor<T>& D_diff   = ttensors.D_diff;
  Tensor<T>& ehf_tmp  = ttensors.ehf_tmp;
  Tensor<T>& ehf_tamm = ttensors.ehf_tamm;

  Matrix&    D_alpha           = etensors.D_alpha;
  Tensor<T>& F_alpha           = ttensors.F_alpha;
  Tensor<T>& FD_alpha_tamm     = ttensors.FD_alpha;
  Tensor<T>& FDS_alpha_tamm    = ttensors.FDS_alpha;
  Tensor<T>& D_alpha_tamm      = ttensors.D_alpha;
  Tensor<T>& D_last_alpha_tamm = ttensors.D_last_alpha;

  Matrix&    D_beta           = etensors.D_beta;
  Tensor<T>& F_beta           = ttensors.F_beta;
  Tensor<T>& FD_beta_tamm     = ttensors.FD_beta;
  Tensor<T>& FDS_beta_tamm    = ttensors.FDS_beta;
  ...
  if(iter >= 1) {
    ...
    if(is_rhf) {
      ...
      scf_diis(ec, chem_env, tAO, F_alpha, F_alpha, err_mat_alpha_tamm, err_mat_alpha_tamm, iter,
               max_hist, scf_data, sys_data.n_lindep, ttensors.diis_hist, ttensors.diis_hist,
               ttensors.fock_hist, ttensors.fock_hist);
    }
    if(is_uhf) {
      ++scf_data.idiis;
      scf_diis(ec, chem_env, tAO, F_alpha, F_beta, err_mat_alpha_tamm, err_mat_beta_tamm, iter,
               max_hist, scf_data, sys_data.n_lindep, ttensors.diis_hist, ttensors.diis_beta_hist,
  ...
  // if(rmsd < 1e-6) { scf_data.switch_diis = true; } // rmsd check
  ...
  auto compute_2body_2index_ints_lambda = [&](const IndexVector& blockid) {
    ...
    for(auto s1 = s1range_start; s1 <= s1range_end; ++s1) {
      ...
      for(auto s2 = s2range_start; s2 <= s2range_end; ++s2) {
        ...
        //   auto s2spl = scf_data.dfbs_shellpair_list.at(s2);
        //   if(std::find(s2spl.begin(),s2spl.end(),s1) == s2spl.end()) continue;
        // }
        // else{
        //   auto s2spl = scf_data.dfbs_shellpair_list.at(s1);
...
void exachem::scf::SCFIter<T>::init_ri(ExecutionContext& ec, const ChemEnv& chem_env,
  ...
  if(!direct) {
    // Compute 3c ints
    Tensor<T>::allocate(&ec, xyZ);
    compute_3c_ints(ec, chem_env, scf_data, xyZ);
...
void exachem::scf::SCFIter<T>::compute_2bf_ri(ExecutionContext& ec, const ChemEnv& chem_env,
  ...
  Tensor<T> Jtmp_tamm{scf_data.tdfAO};                          // ndf
  Tensor<T> tmp_df{scf_data.tAO, scf_data.tAO, scf_data.tdfAO}; // n, n, ndf
...
void exachem::scf::SCFIter<T>::scf_diis(
  ...
  if(ndiis > max_hist) {
    auto maxe = 0;
    if(!scf_data.switch_diis) {
      std::vector<T> max_err(diis_hist_alpha.size());
    ...
    Tensor<T>::deallocate(diis_hist_alpha[maxe]);
    Tensor<T>::deallocate(fock_hist_alpha[maxe]);
    diis_hist_alpha.erase(diis_hist_alpha.begin() + maxe);
    fock_hist_alpha.erase(fock_hist_alpha.begin() + maxe);
    if(is_uhf) {
      Tensor<T>::deallocate(diis_hist_beta[maxe]);
      Tensor<T>::deallocate(fock_hist_beta[maxe]);
  ...
  if(is_uhf) {
    Tensor<T>::allocate(&ec, Fcopy_beta);
  ...
  while(info != 0) {
    if(idim == 1) {
      Tensor<T>::deallocate(dhi_trace);
      return;
    }

    N = idim + 1;
    std::vector<T> AC(N * (N + 1) / 2);
    std::vector<T> AF(AC.size());
    std::vector<T> B(N, 0.);
    B.front() = -1.0;
    X.resize(N);
    ...
    std::vector<T>       BERR(1), FERR(1); // NRHS
    ...
    if(info != 0) {
      if(rank == 0)
        cout
          ...
          << endl;
      Tensor<T>::deallocate(diis_hist_alpha[0]);
      Tensor<T>::deallocate(fock_hist_alpha[0]);
      diis_hist_alpha.erase(diis_hist_alpha.begin());
      fock_hist_alpha.erase(fock_hist_alpha.begin());

      if(is_uhf) {
        Tensor<T>::deallocate(diis_hist_beta[0]);
        Tensor<T>::deallocate(fock_hist_beta[0]);
  ...
  Tensor<T>::deallocate(dhi_trace); // dhi_trans

  std::vector<T> X_final(N);
...
void exachem::scf::SCFIter<T>::compute_2bf_deriv(
  ...
  auto&   Ga_deriv = etensors.Ga_deriv;
  Matrix& D        = etensors.D_alpha;
  auto&   Gb_deriv = etensors.Gb_deriv;
  Matrix& D_beta   = etensors.D_beta;
  ...
  const int nderiv =
    libint2::num_geometrical_derivatives(chem_env.atoms.size(), deriv_order); // total # of derivs
  // const auto ncoords_times_two = (chem_env.atoms.size() * 3) * 2;
  size_t shell_atoms[4];
  auto   shell2atom = chem_env.shells.shell2atom(chem_env.atoms);
  ...
  std::vector<Matrix> J12_vec(nderiv, Matrix::Zero(shblk, shblk)); //, J34_vec;
  std::vector<Matrix> K1_alpha_vec(nderiv, Matrix::Zero(shblk, N)),
    K1_beta_vec(nderiv, Matrix::Zero(shblk, N));
  std::vector<Matrix> K2_alpha_vec(nderiv, Matrix::Zero(shblk, N)),
    K2_beta_vec(nderiv, Matrix::Zero(shblk, N));
  ...
  auto comp_2bf_lambda = [&](IndexVector blockid) {
    ...
    // To avoid skipping over the whole Fock matrix
    J12_vec = std::vector<Matrix>(nderiv, Matrix::Zero(shblk, shblk));
    if(doK) {
      K1_alpha_vec = std::vector<Matrix>(nderiv, Matrix::Zero(shblk, N));
      K2_alpha_vec = std::vector<Matrix>(nderiv, Matrix::Zero(shblk, N));
      if(is_uhf) {
        K1_beta_vec = std::vector<Matrix>(nderiv, Matrix::Zero(shblk, N));
        K2_beta_vec = std::vector<Matrix>(nderiv, Matrix::Zero(shblk, N));
      }
    }
    ...
    for(std::remove_const_t<decltype(s1)> s3 = 0; s3 <= s1; ++s3) {
      ...
      for(const auto& s4: scf_data.obs_shellpair_list.at(s3)) {
        ...
        // compute integrals
        engine.compute2<Operator::coulomb, libint2::BraKet::xx_xx, deriv_order>(
          obs[s1], obs[s2], obs[s3], obs[s4], sp12, sp34);

        if(buf[0] == nullptr) continue; // if all integrals screened out, skip to next quartet
        ...
        for(auto idx = 0; idx != 12; ++idx) {
          ...
          auto&       G        = Ga_deriv[coord];
          auto&       G_beta   = Gb_deriv[coord];
          const auto* buf_1234 = buf[idx];

          auto& J12      = J12_vec[coord];
          auto& K1_alpha = K1_alpha_vec[coord];
          auto& K2_alpha = K2_alpha_vec[coord];
          auto& K1_beta  = K1_beta_vec[coord];
          auto& K2_beta  = K2_beta_vec[coord];
    ...
    for(auto d = 0; d != nderiv; ++d) {
      auto& G      = Ga_deriv[d];
      auto& G_beta = Gb_deriv[d];

      auto& J12      = J12_vec[d];
      auto& K1_alpha = K1_alpha_vec[d];
      auto& K2_alpha = K2_alpha_vec[d];
      auto& K1_beta  = K1_beta_vec[d];
      auto& K2_beta  = K2_beta_vec[d];
  ...
  if(!do_density_fitting) {
    ...
    Tensor<T> Fa_deriv{scf_data.tAO, scf_data.tAO};
    Tensor<T> Fb_deriv{scf_data.tAO, scf_data.tAO};
    sch.allocate(Fa_deriv, Fb_deriv).execute();

    for(auto d = 0; d != nderiv; ++d) {
      sch(Fa_deriv() = 0)(Fb_deriv() = 0).execute();

      auto& G      = Ga_deriv[d];
      auto& G_beta = Gb_deriv[d];
      eigen_to_tamm_tensor_acc(Fa_deriv, G);
      if(is_uhf) eigen_to_tamm_tensor_acc(Fb_deriv, G_beta);
      ec.pg().barrier();
      G = tamm_to_eigen_matrix(Fa_deriv);
      if(is_uhf) G_beta = tamm_to_eigen_matrix(Fb_deriv);
      ec.pg().barrier();
    }
    sch.deallocate(Fa_deriv, Fb_deriv).execute();

    // symmetrize the result
    // if(rank == 0) {
    for(auto d = 0; d != nderiv; ++d) {
      Matrix tmp  = 0.5 * (Ga_deriv[d] + Ga_deriv[d].transpose());
      Ga_deriv[d] = tmp;
      if(is_uhf) {
        tmp         = 0.5 * (Gb_deriv[d] + Gb_deriv[d].transpose());
        Gb_deriv[d] = tmp;
      }
    }
    // }
